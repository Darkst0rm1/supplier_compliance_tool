...
def _display_label(c: str) -> str:
    """H|Sept / F|Sept both become bare 'Sept' if simply stripped, which
    collides into duplicate column names Streamlit/Arrow can't render
    sensibly -- keep them distinguishable instead."""
    if c.startswith("H|"):
        return f"Hist {c[2:].strip()}"
    if c.startswith("F|"):
        return f"Fcst {c[2:].strip()}"
    return c
...
st.caption(
    ...
    "optionally, your current forecast file. Order Quantity is current demand; "
    "Invoice Quantity is what has actually invoiced — shown separately because "
    "open 2026 orders may not be invoiced yet."
)
