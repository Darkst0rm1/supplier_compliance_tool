"""Tests for the Overstock report's optional EWM storage-bin lookup.
...
SLED_FLOOR = date(2026, 7, 6)
LAST_SELL_CUTOFF = date(2026, 7, 7)


def _ewm_book(rows, owner="BP2910"):
    """An EWM stock export as bytes. ``rows`` are (product, batch, bin)."""
...
def _build(mat_rows, ewm_bins=None):
    ...
    return build_overstock(
        materials, master,
        sled_floor=SLED_FLOOR, last_sell_cutoff=LAST_SELL_CUTOFF,
        ewm_bins=ewm_bins,
    )
...
def test_file_uploaded_for_the_wrong_plant_is_rejected():
    with pytest.raises(OverstockError, match="uploaded in the 2910 box"):
...
def test_a_non_ewm_file_is_rejected_by_name():
    ...
    wb.save(buf)
    buf.seek(0)
    with pytest.raises(OverstockError, match="EWM stock"):
...
def test_materialbatch_is_the_key_and_appears_once():
    """The signed-off workbook repeated this column (G and K) as VLOOKUP
    residue. Only the first is kept now the lookup lives in code."""
...
def test_new_columns_sit_where_the_golden_workbook_puts_them():
    assert BASE_COLUMNS[6:8] == [OUT_MATERIALBATCH, OUT_BIN]
    # Batch keeps its old neighbour now the duplicate key column is gone.
    assert BASE_COLUMNS[9:11] == [MAT_BATCH, MAT_SLED]


def test_excel_headers_match_the_signed_off_layout():
    sheets = _build([("10026065", "2910", "2609191170")])
...
def test_autofilter_still_ends_at_blocked_stock():
    sheets = _build([("10026065", "2910", "B1")])
    ws = openpyxl.load_workbook(io.BytesIO(generate_excel(sheets)))["Mississauga"]
